...
**Enter answer here**
- Work: O(n) since we must sum 'n' ones.  
- Span: O(log n) using a parallel tree based reduction. 
...
**Enter answer here**
- The simple dictionary loop works sequentially, but it depends on continuously updating one shared object.  
- This makes it hard to split across multiple processors, since updates could interfere with one another.  
- Map-reduce avoids this by first generating independent key–value pairs and then merging results in parallel, which scales much better.  
